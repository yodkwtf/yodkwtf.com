...
The newsletter is fully wired to **Resend** (free tier: 3,000 emails/mo, 100/day, 1,000 audience contacts). Env vars: `RESEND_API_KEY` + `RESEND_AUDIENCE_ID` (see `.env.example`) — needed on Netlify (subscribe form) and as GitHub Actions secrets (broadcasts).

- **Subscribe**: `src/app/api/newsletter/route.ts` adds the email to the Resend Audience via plain `fetch` (no SDK). Returns 503 if env vars are missing; treats duplicate contacts (409/"exists") as success. `NewsletterSignup.tsx` still has the `COMING_SOON` flag (now `false`) to disable the form if ever needed.
- **Broadcasts**: `scripts/send-newsletter.ts` (run via `npm run newsletter`) finds published posts not yet listed in `scripts/newsletter-sent.json`, builds an inline-styled HTML email (eyebrow → cover image → category pill → title → date/reading-time meta → description → "Read full article" button → circular social-icon badges → `{{{RESEND_UNSUBSCRIBE_URL}}}` footer), and sends one Resend Broadcast per post. Flags: `--dry-run`, `--seed` (mark everything sent without emailing), `--force` (bypass the refuse-to-send-more-than-3 safety check that guards against a lost sent-log). `buildEmailHtml` is exported for previewing. **Avoid bare domain text in the email** (e.g. `yodkwtf.com` as a text node) — Gmail auto-links it into an ugly blue link; wrap it in a styled `<a>` with an explicit color instead.
- **Email icons**: Gmail strips inline SVG, so social icons are pre-rendered PNGs in `public/newsletter/{github,x,linkedin,website}.png`, generated by `scripts/generate-newsletter-icons.mjs` (`npm run newsletter:icons`, uses `sharp`) as near-black circular badges with white glyphs. The email references them by **absolute production URL** (`${siteConfig.url}/newsletter/*.png`), so they only render in a real send **after the site is deployed** with those static assets — a local/pre-deploy test email shows broken icon images (the external cover image still works). To preview the full design offline, render `buildEmailHtml` and rewrite the icon URLs to local `file://` paths.
- **Automation**: `.github/workflows/newsletter.yml` runs on pushes to `main` touching `content/blogs/**`, on a daily cron (04:30 UTC — this is what catches future-dated posts when their `publishedAt` arrives), and manually. It commits `newsletter-sent.json` back with `[skip ci]` (so Netlify doesn't redeploy for it).
- The sent-log is **seeded with all pre-newsletter posts** — never delete it, or the safety check is all that stands between subscribers and a blast of old posts. When adding a post that shouldn't be emailed, add its slug to the JSON manually.
