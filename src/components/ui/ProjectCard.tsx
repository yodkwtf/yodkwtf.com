...
export function ProjectCard({ project, featured, className }: ProjectCardProps) {
  return (
    ...
    >
      ...
      <div className={cn("flex flex-col flex-1 p-5 gap-3", featured && "md:p-7")}>
        <div className="flex items-start justify-between gap-4">
          <h3 className={cn(
            "font-display leading-snug text-ink group-hover:text-accent-500 transition-colors",
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {!project.clientWork && project.githubUrl && (
              <a href={project.githubUrl} target="_blank" rel="noopener noreferrer"
                ...
                className="text-ink-faint hover:text-ink transition-colors p-1" aria-label="GitHub">
                <GithubIcon size={15} />
              </a>
            )}
            {project.liveUrl && (
              <a href={project.liveUrl} target="_blank" rel="noopener noreferrer"
                onClick={(e) => e.stopPropagation()}
                className="text-ink-faint hover:text-accent-500 transition-colors p-1" aria-label="Live site">
        ...
        <p className="text-sm text-ink-muted line-clamp-2 leading-relaxed flex-1">{project.summary}</p>

        <div className="flex flex-wrap gap-1.5 mt-auto">
          {project.techStack?.slice(0, 4).map((tech) => <TagPill key={tech} label={tech} />)}
          {(project.techStack?.length ?? 0) > 4 && (
            <span className="tag-pill text-ink-faint">+{project.techStack.length - 4}</span>
          )}
        </div>
