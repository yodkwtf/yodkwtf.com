...
export function FeaturedProjectCard({ project, index, className }: FeaturedProjectCardProps) {
  ...
  return (
    <motion.article
      ...
      className={cn("group flex flex-col md:flex-row min-h-105 border-b border-border last:border-0", className)}
    >
      {/* ── Image panel (60%) ── */}
      <div
        className={cn(
      ...
      >
        ...
        <div className="flex items-center gap-4 justify-center md:justify-start">
          ...
          >
            Case study <ArrowUpRight size={14} className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
          </Link>
        </div>
      </div>
    </motion.article>
